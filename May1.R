...
airbnb_features <- train_perfect %>%
  select(accommodates, bedrooms, beds, cancellation_policy,has_cleaning_fee, host_total_listings_count, 
         ...
         common_word_name, common_word_neighborhood, accommodates_bedrooms, accomodates_beds, 
         accomodates_yearly, bedrooms_bathrooms, beds_bathrooms, beds_yearly,
         label)
...
predictions <- predict(xgb_model, valid_matrix)
predictions
...
ranger_model <- ranger(high_booking_rate ~ ., data = X_train_ranger, num.trees = 2500, mtry = 5, probability = TRUE)
